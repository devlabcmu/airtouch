...
/*
 added by Xiang 'Anthony' Chen
 */
- (void)addAirPoint: (CGPoint)pnt
{
    [self addTouchAtPoint:pnt];
}

#define MAX_NUM_POINTS kSamplePoints
float xMin = 10000;
int idxMinX = -1;
float xMax = -10000;
int idxMaxX = -1;
...
{
    if(touchPoints.count > MAX_NUM_POINTS) [touchPoints removeObjectAtIndex:0];
	[touchPoints addObject:[NSValue valueWithCGPoint:point]];
    if(point.x < xMin)
    {
        xMin = point.x;
        idxMinX = touchPoints.count - 1;
    }
    if(point.x > xMax)
    {
        xMax = point.x;
        idxMaxX = touchPoints.count - 1;
    }

//    NSLog(@"%d", touchPoints.count);
}

- (BOOL)isReadyForRecognition
{
//    NSLog(@"%d", touchPoints.count);
    return touchPoints.count >= MAX_NUM_POINTS;
}
...
{
    // added by Anthony
    if(touchPoints.count < MAX_NUM_POINTS) return @"";
    //
